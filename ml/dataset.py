...
def get_train_transforms(input_size: int = 48, grayscale: bool = True) -> A.Compose:
    ...
    return A.Compose([
        ...
        A.CoarseDropout(
            max_holes=8, max_height=input_size // 8, max_width=input_size // 8,
            min_holes=1, min_height=2, min_width=2, p=0.3
        ),
        A.Normalize(
        ...
        for emotion_name, emotion_idx in self.EMOTION_FOLDERS.items():
            ...
            if emotion_dir.exists():
                for img_path in emotion_dir.glob("*.jpg"):
                    ...
                    label = FER2013_TO_MOOD[emotion_idx] if map_to_moods else emotion_idx
...
class FER2013CSVDataset(Dataset):
    ...
    def _print_distribution(self):
        names = MOOD_NAMES if self.map_to_moods else ["angry", "disgust", "fear", "happy", "sad", "surprise", "neutral"]
        for i, name in enumerate(names):
...
if __name__ == "__main__":
    ...
    dummy_frames = [np.random.randint(0, 255, (48, 48), dtype=np.uint8) for _ in range(10)]
